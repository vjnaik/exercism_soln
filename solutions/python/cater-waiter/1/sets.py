...
def check_drinks(drink_name, drink_ingredients):
    ...
    """
    type_of_drink = "Cocktail" if any(item in ALCOHOLS for item in drink_ingredients) else "Mocktail"
    return f"{drink_name} {type_of_drink}"
...
def tag_special_ingredients(dish):
    ...
    """
    return (dish[0], set(item for item in dish[1] if item in SPECIAL_INGREDIENTS))
...
def compile_ingredients(dishes):
    ...
    """
    return set(item for dish in dishes for item in dish)
